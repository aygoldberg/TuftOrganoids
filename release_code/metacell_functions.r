...
matplot.2 = function(X, cex_mat, grad = colorRampPalette(c("blue", "white", "red"))(101),
	  zlim = quantile(X, c(0,1), na.rm=T), legends=T, col_n = 5, cex_n = 5, max_size=9,
                    grid=F, cex_lim = quantile(cex_mat, c(0,1), na.rm=T)) {

  hmed = seq(0,1,length.out=nrow(X)); vmed = seq(0,1,length.out=ncol(X))
  col_val = round(100 * (X - zlim[1]) / (zlim[2] - zlim[1])) + 1
  col = grad[col_val]
  min_cex = min(cex_lim); max_cex = max(cex_lim)
  cex_val = (cex_mat - min_cex) / (max_cex - min_cex) * max_size + 0.5
  if (legends) {par( fig = c(0,1,0,0.8))}
  plot(rep(vmed, each = nrow(X)), rep(hmed, ncol(X)), pch=21, bg=col, cex = cex_val,
	axes = F, xlab="", ylab="", xlim = c(-0.1,1.1), ylim = c(-0.02,1.02), yaxs = "i")
  axis(1, at=vmed, labels = colnames(X))
  axis(2, at=hmed, labels = rownames(X), las=2)
  if (grid) { grid(col="black")}
  box()
  if (legends) {
   par(mar = c(3,1,1,1), fig = c(0,0.5,0.8,1), new=T)
   plot(x = seq_len(col_n), y = rep(1, col_n), pch=21, cex = 3,
        bg = grad[ seq(1, length(grad), length.out = col_n)], 
        axes=F, xlab="", ylab="", ylim=c(0.98,1.1), yaxs="i")
   axis(1, at = seq_len(col_n), labels = round(seq(zlim[1], zlim[2], length.out=5), 2))
   par(fig = c(0.5,1,0.8,1), new=T)
   cex_val = (cex_lim - min_cex) / (max_cex - min_cex) * max_size + 0.5  
   plot(x = seq_len(cex_n), y = rep(1, cex_n), pch=21, 
        cex = seq(min(cex_val, na.rm=T), max(cex_val, na.rm=T), length.out = cex_n),
        bg = grad[50], axes=F, xlab="", ylab="", xlim = c(0.9, cex_n * 1.1), ylim=c(0.98,1.1), yaxs="i")
   axis(1, at = seq_len(cex_n), labels = round(seq(min_cex, max_cex, length.out=5), 2))
  }
}
...
read_large_umicount = function(mat_id, bs = 1e4, cells = NULL) { 
	mat = scdb_mat(mat_id)
	if (is.null(cells)) { cells = mat@cells}
	ncells = length(cells)
	umicount = rep(NA, length(cells)); names(umicount) = cells
	for (i in seq_len(ncells %/% bs + 1)) {
		from = (i - 1) * bs + 1
		to = min(i * bs, ncells)
		umicount[from:to] = colSums(as.matrix(mat@mat[,cells[from:to]]))
	}
	umicount
}
...
create_batch_matrix = function(mc_id, mat_id, comb, fname, batch_iden = "amp_batch_id", cord = colnames(sc_cl@mc_fp), clusts = NULL,
        ...
        mar = c(5,10,0,20), zlim = c(0,1), cell_stats = NULL) {

        sc_cl = scdb_mc(mc_id); sc_mat = scdb_mat(mat_id)
        cells = intersect(names(sc_cl@mc), names(comb))
        if (is.null(cell_stats)) {cell_stats = sc_mat@cell_metadata[cells,]}
    ...
    tab_fn, sz_cor_thresh = NA, downsample_n = NA, cells = NULL)
{
    ...
    mat_ds = as.matrix(scm_downsamp(umis, downsample_n))
...
explore_gene_set = function(mc_id, mat_id, cells, ct, outdir, prefix, comb,
	...
	modules_c = log2((1 + modules_c) / (1 + apply(modules_c, 1, median)))
...
mcell_plot_gset_cor_mats_no_file = function (gset_id, scmat_id, downsamp = T) 
{
    feat = gset_get_feat_mat(gset_id, scmat_id, downsamp = downsamp)
    gset = scdb_gset(gset_id)
    k_nonz_exp = 7 #get_param("scm_k_nonz_exp")
    feat = as.matrix(log2(1 + k_nonz_exp * feat))
    mat = scdb_mat(scmat_id)
    cnms = colnames(feat)
    umis = read_large_umis(scmat_id, cells = cnms)
    csize = colSums(umis)
    ...
    for (s_id in s_ids) {
        ...
        shades = colorRampPalette(c("darkblue", "blue", "white", 
            "red", "yellow"))(1000)
        breaks = seq(-0.5, 0.5, l = 1001)
        vs = pmin(pmax(gcor[s_nms, s_nms], -0.5), 0.5)
        pheatmap::pheatmap(vs, col = shades, breaks = breaks, 
            cluster_rows = n > 2, cluster_cols = n > 2, main = s_id)
        #title(s_id)
        #dev.off()
    }
}

as.matrix.large = function(X, by = "col", bs = 1e4) {
    mat = NULL
    if (by == "col" ) {
        n = ncol(X)
        for (i in seq_len(n %/% bs + 1)) {
            from = (i - 1) * bs + 1
            to = min(i * bs, n)
            mat = cbind(mat, as.matrix(X[, from:to]))
        }
    } else {
        n = nrow(X)
        for (i in seq_len(n %/% bs + 1)) {
            from = (i - 1) * bs + 1
            to = min(i * bs, n)
            mat = rbind(mat, as.matrix(X[from:to,]))
        }
    }    
    mat
}
    ...
    tab_fn, sz_cor_thresh = NA, downsample_n = NA, cells = NULL) 
{
    ...
    mat_ds = as.matrix.large(scm_downsamp(umis, downsample_n))
    mat_ds = mat_ds[rowSums(mat_ds) > 10, ]
    csize = colSums(umis[, colnames(mat_ds)])
    gcors = data.frame(sz_cor = apply(mat_ds, 1, cor, csize))
    gene_anchors = intersect(gene_anchors, rownames(gcors))
...
import_metacell_structure = function(id, folder=id, all_id="all", bad_genes = c(), url=NULL) {
	...
	all_cells = names(sin_mc)
	mcell_mat_ignore_genes(new_mat_id=id, mat_id=all_id, bad_genes, reverse=F)
	
	bad_cells = setdiff(all_mat@cells, all_cells)
	mcell_mat_ignore_cells(id, id, union(bad_cells, all_mat@ignore_cells))

	mat = scdb_mat(id)
	sin_cl = tgMCCov(sin_mc, setdiff(mat@cells, names(sin_mc)), mat)
	...
	sin_cgraph = tgCellGraph(cgraph, mat@cells)
